...
- Graphs retain their kNN source provenance; community results retain
  both graph and upstream provenance.
- Gaussian graphs record the effective bandwidth chosen when
  `sigma = NULL`, and Leiden results record the effective
  `n_iterations`.
- Compatible neighbour lists now validate their optional device
  metadata.
