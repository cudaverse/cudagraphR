...
`cudagraphR` is the graph layer of the **cudaverse**. It turns GPU-aware
nearest-neighbour results into sparse R graphs and provides a stable
...
0.2.0, graph assembly and community detection run on the CPU; the
...
The [backend and provenance
article](https://cudaverse.github.io/cudagraphR/articles/backend-provenance.html)
shows how to inspect both histories, includes a runnable CPU workflow,
and documents the optional CUDA hardware gate.
...
This installs graph construction and the optional upstream kNN
integration. Community detection additionally requires `igraph`:
...
Graph construction does not require `igraph`. Once the optional package
is installed, cluster the same graph:
...
for the full counts-to-neighbours-to-graph path and its matrix
orientation.
